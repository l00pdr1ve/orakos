...
interface NumerologyDisplayViewProps {
...
const LoadingSpinner: React.FC = () => (
    <div className="flex flex-col items-center justify-center gap-4 text-center">
        ...
        <h2 className="font-serif text-2xl font-bold text-text-secondary animate-pulse">Calculando tus números...</h2>
        <p className="text-text-secondary">Tu perfil numerológico se está materializando.</p>
    </div>
);


const NumerologyDisplayView: React.FC<NumerologyDisplayViewProps> = ({ numerologyData, navigateHome }) => {
    ...
    useEffect(() => {
        const fetchReport = async () => {
            if (!numerologyData) {
                setError("No se han proporcionado datos. Por favor, vuelve a empezar.");
                setIsLoading(false);
                return;
            }
            try {
                const result = await generateNumerologyReport(numerologyData);
    ...
    return (
        <div className="max-w-4xl mx-auto animate-fade-in">
            ...
            {!isLoading && !error && reportResult && (
                <div className="mt-10">
                    <SendToEmail content={reportResult} initialEmail={numerologyData?.email} />
...
export default NumerologyDisplayView;