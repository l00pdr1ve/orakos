...
import { ChatMessage as ChatMessageType, LenormandCard } from '../../types';
import { LENORMAND_DECK } from '../../constants';
import { startLenormandChat } from '../../services/geminiService';
import ChatMessage from '../ChatMessage';
import LenormandCardDisplay from '../LenormandCardDisplay';
...
interface LenormandChatViewProps {
    navigateHome: () => void;
}
...
const LenormandChatView: React.FC<LenormandChatViewProps> = ({ navigateHome }) => {
    ...
    const handleSendMessage = useCallback(async (e: React.FormEvent) => {
        ...
        if (!questionAsked) {
            const cards = shuffleDeck(LENORMAND_DECK).slice(0, 3);
            setDrawnCards(cards);
        ...
        try {
            ...
            setMessages(prev => [...prev, { role: 'model', content: "Lo siento, ha ocurrido un error al consultar el oráculo. Por favor, intenta de nuevo más tarde." }]);
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <div className="max-w-4xl mx-auto h-full flex flex-col animate-fade-in">
            ...
            >
                {drawnCards.length > 0 && <LenormandCardDisplay cards={drawnCards} />}
...
export default LenormandChatView;