...
import { EnneagramData } from '../../types';
import { generateEnneagramReport } from '../../services/geminiService';
import ReactMarkdown from 'react-markdown';
import SendToEmail from '../SendToEmail';

interface EnneagramDisplayViewProps {
...
const LoadingSpinner: React.FC = () => (
    <div className="flex flex-col items-center justify-center gap-4 text-center">
        ...
        <h2 className="font-serif text-2xl font-bold text-text-secondary animate-pulse">Analizando tu personalidad...</h2>
        <p className="text-text-secondary">Tu perfil del Eneagrama se está revelando.</p>
    </div>
);


const EnneagramDisplayView: React.FC<EnneagramDisplayViewProps> = ({ enneagramData, navigateHome }) => {
    ...
    useEffect(() => {
        const fetchReport = async () => {
            if (!enneagramData) {
                setError("No se han proporcionado datos del cuestionario. Por favor, vuelve a empezar.");
                setIsLoading(false);
                return;
            }
            try {
                const result = await generateEnneagramReport(enneagramData);
                setReportResult(result);
            } catch (err) {
                setError("Ha ocurrido un error inesperado al generar tu perfil. Por favor, inténtalo de nuevo.");
                console.error(err);
            } finally {
                setIsLoading(false);
            }
        ...
        if (error) {
            return (
                <div className="text-center text-red-800 bg-red-100 border border-red-300 p-8 rounded-lg">
                    <h2 className="font-serif text-2xl font-bold text-red-800 mb-2">Error al generar el perfil</h2>
                    <p>{error}</p>
                </div>
            )
        }
...
export default EnneagramDisplayView;